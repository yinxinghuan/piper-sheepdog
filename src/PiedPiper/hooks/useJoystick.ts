...
export function useJoystick(enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const onDown = (e: PointerEvent) => {
      ...
      // Skip pointerdowns on interactive UI (dropdowns, buttons, leaderboard).
      // Without this, tapping the behavior <select> would also drag the dog.
      const tgt = e.target as Element | null;
      if (tgt && tgt.closest('select, button, [data-no-joystick]')) return;
